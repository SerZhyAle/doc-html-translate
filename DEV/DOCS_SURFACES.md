...
- **3 languages** - `en ru uk`. Long-form prose that is rewritten often or read by store reviewers:
  ...
  material is worse than none. Strategic decisions D3 and D4 of the thirteen-language ticket
  `2026-07-28_thirteen-ui-languages.md`, which was never committed to the repository.
...
| Sitemap / robots | `sitemap.xml` · `robots.txt` | n/a | The landing cluster (root, `?l=ru`, `?l=uk`, ten locale pages) carries all 14 `hreflang` alternates; each child page carries its own cluster (docs trio; `extension.html`, `privacy.html`, `extension-privacy.html` each with `?l=ru` / `?l=uk`). **`sitemap.xml` is generated, never hand-edited**: a page declares its address (`<link rel="canonical">` plus its `hreflang` cluster), its record in `docs/DOCUMENT_REGISTRY.jsonl` decides whether it is announced, and `scripts/doc-registry.ps1 -Generate` writes the sitemap; the same script in `scripts/check.ps1` fails on a stale sitemap, an unregistered page, or an announced page without the full SEO block. |
| Site kit and shared assets | `assets/sza-kit.css` · `assets/site.css` · `assets/site.js` | n/a | The kit is a byte copy of the `PAGE-STYLE` reference (SHA-256 pinned in `tests/site_test.go`) - never edit it; page rules go in `site.css`, shared scripts in `site.js`. Every page lists the full `SITE-FAMILY-MAP` footer; a new sibling means every page plus `familyURLs` in the test. `.sza-canon.json` `site.pages` lists every page. |
| Extension page | `extension.html` | en/ru/uk in-page | Browser-extension landing section. Self-referencing `hreflang` (`?l=ru` / `?l=uk`). |
...
| Install trust page | `install-trust.html` + the `#smartscreen` note in `index.html` and the ten `<code>/index.html` · the "Download" paragraph of the README trio · the release link in the `docs.*` trio | en/ru/uk in-page (page) · **13** (the landing note) | `INSTALL-TRUST` 1.0 (`docs/contracts/INSTALL-TRUST.md`): four sections in order, dialog quoted, no instruction that weakens a protection. "What the app never does" must say what `privacy.html` says - change them together. When builds become signed, update the page, never delete it. |
| Store privacy | `extension/store/PRIVACY.md` · `privacy.html` · `extension-privacy.html` · the permission justifications in `extension/store/LISTING.md` · the runFullTrust text in `msix/README.md` | en (store texts) · en/ru/uk in-page (pages) | Only when data handling changes. **The facts are rows, not prose**: what each edition accesses, sends, writes and declares lives in `docs/security-posture.json`, and the marked blocks of these files are rendered from it (`scripts/security-posture.ps1 -Render`); `scripts/check.ps1` fails on a hand-edited block, a manifest permission without a row, or a network call no row covers. The prose around the blocks is authored, and the English is what the stores reviewed. Contact is `sza@ukr.net` only. |
