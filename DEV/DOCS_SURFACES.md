...
| Store listing sources | `tools/store/listing/<code>.txt` × 13 | **13** | Title, short description, description, features, search terms. `@@ReleaseNotes` only in `en`, `ru`, `uk`. Rendered into the CSV by `tools/store/build-store-listing-csv.ps1` - never hand-edit `listingData.csv`. |
...
| Extension screenshots | `extension/store/screenshots/*.png` | en / ru | Still hand-made; no per-language generator yet (phase 07 step 07.5). |
