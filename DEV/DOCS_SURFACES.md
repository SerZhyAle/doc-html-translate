...
| Sitemap / robots | `sitemap.xml` · `robots.txt` | n/a | The landing cluster (root, `?l=ru`, `?l=uk`, ten locale pages) carries all 14 `hreflang` alternates; each child page carries its own cluster (docs trio; `extension.html`, `privacy.html`, `extension-privacy.html` each with `?l=ru` / `?l=uk`). Regenerate when a page is added. |
...
| Store privacy | `extension/store/PRIVACY.md` · `privacy.html` · `extension-privacy.html` | en (source) · en/ru/uk in-page (pages) | Only when data handling changes; the English text is what the stores reviewed. Contact is `sza@ukr.net` only. |
