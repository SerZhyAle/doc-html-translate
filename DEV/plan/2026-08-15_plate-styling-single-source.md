...
|---|---|---|
| CLI (`doc-html-translate`) | `[ ]` | owns the desktop copy of the plate description and of the palette; must end up deriving both |
| GUI (`doc-html-ui`) | `[ ]` | inherits the pipeline; no new flag, no surface change |
| MSIX Store app | `[ ]` | inherits the GUI; no packaging impact expected - confirm nothing new must ship inside the package |
| Browser extension | `[ ]` | owns the second copy of both; the ring is already removed, the image guard still has to move to the role |
| Website / docs | `[ ]` | `docs/PARITY.md` gains the shared-appearance row, the named-divergence list and the guarded / prose-only mark on every invariant; the catalog's `OCR-PIPELINE.md` points at the single source instead of at two files |
