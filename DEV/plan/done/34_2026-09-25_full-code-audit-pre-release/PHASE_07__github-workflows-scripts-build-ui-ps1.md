# Phase 07: S07 - .github/workflows .. scripts/build-ui.ps1

**Slice:** `S07` in [`slices.json`](slices.json) · **Lines:** 2978 in 19 files ·
**Added since `41fbc1b`:** 1044 · **Risk:** 30.8
...
|------|------:|------:|
| `.github/workflows/publish-cws.yml` | 53 | 0 |
| `.github/workflows/publish-edge.yml` | 49 | 0 |
| `.github/workflows/release.yml` | 246 | 25 |
| `extension/build.mjs` | 294 | 28 |
| `extension/manifest.json` | 68 | 12 |
| `extension/scripts/bump-version.mjs` | 35 | 0 |
| `extension/scripts/gen-appearance.mjs` | 125 | 125 |
| `extension/scripts/publish-cws.mjs` | 149 | 0 |
| `extension/scripts/publish-edge.mjs` | 85 | 0 |
| `installer/doc-html-translate.iss` | 178 | 1 |
| `msix/AppxManifest.xml` | 132 | 3 |
| `msix/build-msix.ps1` | 200 | 39 |
| `scripts/add_to_dev_log.ps1` | 98 | 0 |
| `scripts/audit-slices.ps1` | 792 | 792 |
| `scripts/bootstrap-tools.ps1` | 15 | 0 |
| `scripts/build-extension.ps1` | 85 | 0 |
| `scripts/build-installer.ps1` | 155 | 4 |
| `scripts/build-local.ps1` | 104 | 8 |
| `scripts/build-ui.ps1` | 115 | 7 |

None - this slice holds its units whole.

Previous-register ids this slice re-checks: B15, B29.
...
- [x] Run the mechanical pass over this slice's own files and paste the verdict lines into the
  ...
  - Other files (.github/workflows/publish-cws.yml .github/workflows/publish-edge.yml .github/workflows/release.yml extension/build.mjs extension/manifest.json extension/scripts/bump-version.mjs extension/scripts/gen-appearance.mjs extension/scripts/publish-cws.mjs extension/scripts/publish-edge.mjs installer/doc-html-translate.iss msix/AppxManifest.xml msix/build-msix.ps1 scripts/add_to_dev_log.ps1 scripts/audit-slices.ps1 scripts/bootstrap-tools.ps1 scripts/build-extension.ps1 scripts/build-installer.ps1 scripts/build-local.ps1 scripts/build-ui.ps1): PowerShell - `Invoke-ScriptAnalyzer` when installed, else a parse
...
### Step 07.2 - Re-check the previous register
...
- [x] For each id listed above, find the fixed code and decide: `still fixed`, `regressed`,
      ...
      `FINDINGS.md`: `- <id> - <verdict> - S07 - <file:line or test that shows it>`.
- **Verification:** `./scripts/audit-slices.ps1 -Summary` no longer lists these ids as missing.
  A `regressed` crit/high becomes a package-1 ticket at priority 90 (Step 07.4).
...
- **Verification:** `./scripts/audit-slices.ps1 -Summary` shows `S07` as closed.
...
Audited 2026-09-26 by a parallel auditor. Read at 26605b9 + the uncommitted working tree; line numbers cite that tree.

**Prescan** (no Go packages and no extension modules in this slice; PSScriptAnalyzer not installed; no parity pair):
- PowerShell `Parser::ParseFile` on the 8 `.ps1` files - 0 parse errors each.
- `node --check` on `build.mjs`, `bump-version.mjs`, `gen-appearance.mjs`, `publish-cws.mjs`, `publish-edge.mjs` - exit 0 each; `manifest.json` parses as JSON.
- `node scripts/gen-appearance.mjs --check` - exit 0.
- `./scripts/audit-slices.ps1` - exit 0, `audit-slices: PASS (21 slices over 237 files)`.
- `./scripts/audit-slices.ps1 -Summary` - exit 1, `audit-slices: FAIL (campaign open: 21 slices open, 144 re-checks missing)` (expected mid-campaign).
- `go test -count=1 -run TestAuditSlices ./tests/` - exit 0, `ok doc-html-translate/tests 11.197s`.
- `node --test test/content-security.test.mjs` (pins the B15 manifest list) - exit 0, `fail 0`.
- Workflows (YAML) read by hand.

**Re-check** (verdicts for FINDINGS.md):
- B15 - still fixed - S07 - `extension/manifest.json:46-67` lists only viewer.html, ocr-host.html, ocr-plates.js; `test/content-security.test.mjs:28-34` pins it (exit 0).
- B29 - still fixed - S07 - `extension/build.mjs:28-33,124-143` SHA-256 pin of the decompressed model plus a 60 s timeout. The desktop-side twin is not fixed: see S07-3.

**Files read in full:** all 19 listed files. For context only: `extension/scripts/_lib.mjs`, `extension/package.json`,
`scripts/release.ps1:125-190`, `internal/ocr/tessdata.go:30-140`, `internal/windowsreg/register_windows.go:200-429`,
`DEV/RELEASE.md:30-111`, tickets 23, 27, 35, done/13.

**Findings:** 1 high, 5 med, 10 low - listed in the orchestrator report (ids S07-1 .. S07-16); no inline fix was made.
