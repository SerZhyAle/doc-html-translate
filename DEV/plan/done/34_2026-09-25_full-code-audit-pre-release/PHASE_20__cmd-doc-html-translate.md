# Phase 20: S20 - cmd/doc-html-translate

**Slice:** `S20` in [`slices.json`](slices.json) · **Lines:** 112 in 2 files ·
**Added since `41fbc1b`:** 10 · **Risk:** 2.9
...
|------|------:|------:|
| `cmd/doc-html-translate/main.go` | 76 | 8 |
| `cmd/doc-html-translate/versioninfo.json` | 36 | 2 |

None - this slice holds its units whole.

Previous-register ids this slice re-checks: P18.
...
- [x] Run the mechanical pass over this slice's own files and paste the verdict lines into the
      handoff notes:
  - Go packages (./cmd/doc-html-translate): `go vet <pkgs>`, `$env:GOOS='linux'; go vet <pkgs>` (the
  ...
  - Other files (cmd/doc-html-translate/versioninfo.json): PowerShell - `Invoke-ScriptAnalyzer` when installed, else a parse
...
### Step 20.2 - Re-check the previous register
...
- [x] For each id listed above, find the fixed code and decide: `still fixed`, `regressed`,
      ...
      `FINDINGS.md`: `- <id> - <verdict> - S20 - <file:line or test that shows it>`.
- **Verification:** `./scripts/audit-slices.ps1 -Summary` no longer lists these ids as missing.
  A `regressed` crit/high becomes a package-1 ticket at priority 90 (Step 20.4).
...
- **Verification:** `./scripts/audit-slices.ps1 -Summary` shows `S20` as closed.
...
- `go vet ./cmd/doc-html-translate/` - exit 0 - (no output)
- `$env:GOOS='linux'; go vet ./cmd/doc-html-translate/` - exit 0 - (no output)
- `golangci-lint run --config configs/.golangci.yml ./cmd/doc-html-translate/` - exit 0 - (no output)
- `go test ./cmd/doc-html-translate/` - exit 0 - `? doc-html-translate/cmd/doc-html-translate [no test files]`
- `gofmt -l cmd/doc-html-translate` - exit 0 - (clean)
- versioninfo.json: `ConvertFrom-Json` parse - ok (Invoke-ScriptAnalyzer not installed; not a parity pair).
  The three IconPath ICOs exist; goversioninfo v1.7.0 (local exe and CI pin) splits IconPath on commas
  and honours ApplicationIconPath; the order matches windowsreg iconMark/iconVerb/iconDocumentType (0/1/2).

Re-check: P18 - still fixed - `internal/config/flags.go:24,118` (`ErrVersion` sentinel),
`cmd/doc-html-translate/main.go:26` (`errors.Is`), `internal/config/flags_test.go:230`.

Files read in full: [x] main.go (76) [x] versioninfo.json (36).

Findings: med 1 (S20-1: the fatal error of a failed run goes to stderr only, never into the run log -
`main.go:30,56`; ticket). crit 0, high 0, low 0.
