# Phase 06: S06 - internal/limits .. internal/mobi

**Slice:** `S06` in [`slices.json`](slices.json) · **Lines:** 880 in 8 files ·
**Added since `41fbc1b`:** 402 · **Risk:** 36.4
...
|------|------:|------:|
| `internal/limits/limits.go` | 109 | 109 |
| `internal/limits/tiff.go` | 69 | 69 |
| `extension/src/limits.js` | 80 | 80 |
| `internal/logging/log.go` | 182 | 29 |
| `internal/md/extract.go` | 178 | 45 |
| `extension/src/md.js` | 40 | 2 |
| `internal/mobi/extract.go` | 88 | 15 |
| `extension/src/ebook.js` | 134 | 53 |

None - this slice holds its units whole.

Previous-register ids this slice re-checks: P21, E22, X9, Q4.
...
- [x] Run the mechanical pass over this slice's own files and paste the verdict lines into the
      handoff notes:
  - Go packages (./internal/limits ./internal/logging ./internal/md ./internal/mobi): `go vet <pkgs>`, `$env:GOOS='linux'; go vet <pkgs>` (the
    non-Windows twins), `golangci-lint run <pkgs>` (it carries the staticcheck analyzers),
    `go test <pkgs>`.
  - Extension modules (extension/src/limits.js extension/src/md.js extension/src/ebook.js): every `extension/test/*.test.mjs` that imports one of them -
...
### Step 06.2 - Re-check the previous register
...
- [x] For each id listed above, find the fixed code and decide: `still fixed`, `regressed`,
      ...
      `FINDINGS.md`: `- <id> - <verdict> - S06 - <file:line or test that shows it>`.
- **Verification:** `./scripts/audit-slices.ps1 -Summary` no longer lists these ids as missing.
  A `regressed` crit/high becomes a package-1 ticket at priority 90 (Step 06.4).
...
- **Verification:** `./scripts/audit-slices.ps1 -Summary` shows `S06` as closed.
...
Run as one of several parallel auditors: `FINDINGS.md`, `INDEX.md` and `RELEASE_QUEUE.md` are left to
the orchestrator, which receives the lines below. Line numbers cite the working tree after the two
inline fixes.

**Prescan** (all green; before and after the inline fixes):

- `go vet ./internal/limits ./internal/logging ./internal/md ./internal/mobi` - exit 0, no output.
- `$env:GOOS='linux'; go vet <same pkgs>` - exit 0, no output.
- `golangci-lint run --config configs/.golangci.yml <same pkgs>` - exit 0, no output.
- `go test -count=1 <same pkgs>` - exit 0, last line `ok doc-html-translate/internal/mobi`.
- `node --test test/content-security.test.mjs test/ebook.test.mjs test/limits.test.mjs` (from
  `extension/`) - exit 0, `pass 15, fail 0`.

**Re-check:**

- P21 - still fixed - S06 - `internal/logging/log.go:54-64` (`emit` holds `runLogMu` across the console write); `TestEmitSerializesConsoleWrites`.
- E22 - still fixed (md half; htmlconv half is another slice) - S06 - `internal/md/extract.go:155` (no `lang`), `:42-49` (local images copied via `assets.Copier`).
- X9 - still fixed (mobi half) - S06 - `internal/mobi/extract.go:64-69` (`procrun.Run` with `Timeout: procrun.Calibre.ForFile`).
- Q4 - still fixed - S06 - `internal/mobi/extract.go:48` (lowercase "calibre ..").

**Findings:**

| id | sev | conf | finding | where | disposition |
|---|---|---|---|---|---|
| S06-1 | low | conf | `CopyCapped` dropped the probe read's error: an entry of exactly `limit` bytes whose reader fails only at end of stream (archive/zip `ErrChecksum`/`ErrFormat`) was reported as a clean copy | `internal/limits/limits.go:90-101` | inline: error now returned; `copycapped_probe_test.go` (3 tests, fail without the fix, pass with it; `go test -count=1 ./internal/limits` exit 0) |
| S06-2 | low | conf | `RunLogf` skipped `runLogFilter`, so panic values/stacks reached the on-disk run log unredacted, against `SetRunLogFilter`'s "every line" (the report re-redacts at pack time, so only the local file is affected) | `internal/logging/log.go:90-100` | inline: filter applied; `runlogf_filter_test.go` (fails without, passes with; `go test -count=1 ./internal/logging` exit 0) |
| S06-3 | low | conf | Go `splitBySections` splits at every `<h1`/`<h2` substring, including a heading nested in a blockquote or list item, yielding a page ending in an open `<blockquote>` and the next starting with a stray `</blockquote>`; `md.js` splits only at top-level headings - Go/JS drift | `internal/md/extract.go:115-147`, `extension/src/md.js:23-31` | ticket (parity pair) |
| S06-4 | low | conf | `retargetBookLinks` strips a document-supplied `data-dht-target` but returns the original html when the section has no book links, so the marker survives the sanitizer and `applyBookLinks` turns it into `href="#<value>"` (fragment only, not scriptable) | `extension/src/ebook.js:51,61,64-69` | ticket |
| S06-5 | low | plaus | Markdown is read whole and rendered with no input budget; a multi-hundred-MB `.md` can exhaust the 386 address space (fatal, not a recoverable panic) | `internal/md/extract.go:27,37` | ticket |
| S06-6 | low | plaus | `ebook-convert` runs under `context.Background()`, so cancelling the run does not stop Calibre until its deadline; same pattern in comic/pdf/ocr | `internal/mobi/extract.go:64` | ticket (cross-slice) |

**Files read in full:** `internal/limits/limits.go`, `internal/limits/tiff.go`, `extension/src/limits.js`,
`internal/logging/log.go`, `internal/md/extract.go`, `extension/src/md.js`, `internal/mobi/extract.go`,
`extension/src/ebook.js`. Limits values and messages match on both sides (`MaxArchiveEntries` 20000,
4 GB total, `FormatBytes` rounding identical).
