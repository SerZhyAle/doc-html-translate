# Phase 22: S22 - extension (build + publish)

**Slice:** `S22` in [`slices.json`](slices.json) · **Lines:** 100 in 2 files ·
**Added since `41fbc1b`:** 2 · **Risk:** 1.4
...
|------|------:|------:|
| `extension/package.json` | 32 | 2 |
| `extension/scripts/_lib.mjs` | 68 | 0 |
...
- [x] Run the mechanical pass over this slice's own files and paste the verdict lines into the
  ...
  - Other files (extension/package.json extension/scripts/_lib.mjs): PowerShell - `Invoke-ScriptAnalyzer` when installed, else a parse
...
### Step 22.2 - Re-check the previous register
...
- [x] For each id listed above, find the fixed code and decide: `still fixed`, `regressed`,
      ...
      `FINDINGS.md`: `- <id> - <verdict> - S22 - <file:line or test that shows it>`.
- **Verification:** `./scripts/audit-slices.ps1 -Summary` no longer lists these ids as missing.
  A `regressed` crit/high becomes a package-1 ticket at priority 90 (Step 22.4).
...
- **Verification:** `./scripts/audit-slices.ps1 -Summary` shows `S22` as closed.
...
Tail slice, cut with `-Tail` after the S07 auditor found these two files in no class (R30), and read in
full by the orchestrating session.

- Prescan: `node --check extension/scripts/_lib.mjs` - exit 0; `extension/package.json` parses.
- Files read: `extension/scripts/_lib.mjs` (68 lines), `extension/package.json` (32 lines).
- Findings: R37 (low - `httpText` has no timeout), R38 (low - `release:cws|edge` runs no `npm test`).
  No crit, high or med; no ticket.