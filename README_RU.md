...
- **Расширение для браузера** - расширение Chromium MV3, которое перерисовывает документы (PDF, EPUB,
  ...
  Оно также распознаёт текст **на всех картинках обычной веб-страницы** по правому клику и кладёт его
  поверх картинок настоящим текстом, не уходя со страницы - веб-комикс или скан архива сохраняют вёрстку,
  ссылки и порядок чтения, а «Перевести страницу» переводит распознанное вместе со всем остальным.
...
**«Windows protected your PC» / «Система Windows защитила ваш компьютер»?** Установщик и portable-программы
не подписаны сертификатом разработчика, поэтому Windows SmartScreen может остановить первый запуск: нажмите
**Подробнее**, затем **Выполнить в любом случае**. Версия из Microsoft Store подписана Microsoft и этого окна
не показывает. Почему так и чего приложение никогда не делает:
[install-trust.html](https://serzhyale.github.io/doc-html-translate/install-trust.html?l=ru).
...
|------|--------------|----------|
| `-register` | `false` | Стать обработчиком по умолчанию в HKCU для всех поддерживаемых типов (выключено; первый запуск спрашивает и записывает только после согласия) |
| `-unregister` | `false` | Отпустить ассоциацию по умолчанию (пункт «Преобразовать в HTML» и «Открыть с помощью» остаются) |
| `-register-openwith` | `false` | Добавить программу в список «Открыть с помощью» и пункт «Преобразовать в HTML», не делая её обработчиком по умолчанию (в GUI `doc-html-ui` то же самое - переключатель в разделе «Интеграция с Windows»; сам по себе он ничего не добавляет) |
| `-notranslate` | `false` | Только конвертация, без перевода |
| `-noopen` | `false` | Не открывать браузер после конвертации (пакетный режим: предупреждения, например об отсутствии поддержки JPEG2000, пишутся в журнал, а не показываются окном) |
...
| `-max-cost` | `0` | Отменить платный перевод до отправки, если оценка в долларах превышает N; в пределах N перевод идёт без окна подтверждения (`0` - без ограничения; отрицательное, NaN или Inf отклоняется) |
...
- Готовый результат открывается без повторной конвертации, только если сделавший его запуск завершился, документ тот же (путь, размер и время изменения) и совпадают настройки, влияющие на результат: движок перевода, `-src`/`-dst`, модель Ollama, `-ocr`/`-ocr-lang`, `-multipage`, `-split` и `-toc-depth`. Иначе результат собирается заново, и в журнале сказано почему (прерван, документ изменился, другие настройки, переведён лишь частично). `-force` пересобирает всегда. Страницы записываются атомарно, поэтому прерванный запуск не оставляет недописанных страниц.
- Коды выхода: `0` - готово, `1` - неверные аргументы, `2` - ошибка ввода-вывода, `3` - документ не удалось разобрать, `4` - перевод не удался, остановился на полпути или не начался, потому что выбранный движок недоступен - нет ключа Google, Ollama не запущена (книга всё равно собрана и открыта, текст на исходном языке; частичный перевод сообщает `переведено частично, N из M страниц`, а сегмент, который движок вернул пустым, считается непереведённым), `130` - прервано по Ctrl+C (следующий запуск соберёт результат заново).
- Стоимость платного перевода оценивается как `символы / 1e6 * $20`, в символах (не байтах), по всему, что отправляется: страницам, названию книги и пунктам оглавления. `-max-cost N` - жёсткая проверка до отправки при любом размере документа: если оценка больше `N`, перевод пропускается, а книга всё равно собирается без перевода. Заданный предел - это и есть согласие: оценка в пределах `N` переводится без окна подтверждения, поэтому запуск без присмотра или из скрипта (`-google -max-cost 2 -noopen book.epub`) не останавливается на вопросе. Без `-max-cost` окно спрашивает обо всём, что длиннее 1000 символов, и по умолчанию ответ - «не тратить»: и Enter, и Escape отказываются. При запуске из GUI вопрос задаётся в окне самого GUI. Отрицательный, `NaN` или бесконечный `-max-cost` отклоняется при запуске (код выхода `1`).
- Ключ Google API передаётся в заголовке запроса, а не в адресе, и вырезается из текста ошибок и из журнала запуска.
- В каждой папке результата лежит скрытая запись о владельце (`.doc-html-translate.json`). Конвертер переиспользует, пересобирает (`-force`) или удаляет после сбоя только папку, которую сам создал для этого же документа. Ваша собственная папка с тем же именем, что и книга, не трогается никогда: результат тогда попадает в соседнюю, например `book (pdf)`. Так же разводятся `book.epub` и `book.pdf`, лежащие рядом.
- На вход нужен файл документа: папка или файл нулевого размера отклоняются до того, как что-либо записано. Вторая конвертация в тот же результат, пока идёт первая, отклоняется, а не смешивается с ней.
- Язык интерфейса не влияет на язык готового документа.
- Текст PDF извлекает `pdftotext` (Poppler), он входит в Windows-приложение. **Ничего не устанавливается автоматически.** Если антивирус заблокировал встроенную копию, конвертер берёт Poppler, установленный вами; иначе переходит на встроенный ридер и подсказывает, как поставить Poppler вручную: `winget install ossia.poppler` в Windows, пакет `poppler-utils` в Linux, `brew install poppler` в macOS. Встроенная копия распаковывается для каждой версии приложения в `%LOCALAPPDATA%\doc-html-translate\pdftotext-<hash>\`, так что после обновления всегда используется новая; старые папки удаляются.
- Каждая внешняя программа (pdftotext, Tesseract, Calibre, 7-Zip, ffmpeg/ImageMagick) работает с ограничением по времени, которое растёт с размером входного файла. Когда оно истекает, программа и всё, что она запустила, останавливаются, а шаг переходит на запасной путь или завершается сообщением с именем программы. На медленной машине или для необычно тяжёлого файла задайте переменную окружения `DOCHT_TOOL_TIMEOUT_SCALE` - множитель: например, `set DOCHT_TOOL_TIMEOUT_SCALE=3` даёт втрое больше времени.
- Пределы входных данных, чтобы один враждебный или огромный файл не исчерпал память или временный диск (у 32-битной сборки адресное пространство 2 ГБ). Изображение декодируется, только если его заголовок объявляет не больше 100 мегапикселей и не больше 32768 пикселей по стороне: более крупный TIFF отклоняется с сообщением, где назван предел, а более крупная картинка на странице всё равно показывается как есть, и Tesseract читает её без подбора цвета плашек и повторных проходов. Архив (EPUB, CBZ, CBT, CBR, CB7) проверяется по списку содержимого до распаковки: не больше 20000 записей и 4 ГБ в распакованном виде, иначе он отклоняется; отдельный файл больше 100 МБ в EPUB или страница комикса больше 200 МБ пропускается с предупреждением, где назван файл. CBR/CB7 сначала перечисляются через 7-Zip, и распаковываются только принятые страницы. Документ TXT, Markdown, FB2, RTF или HTML больше 100 МБ отклоняется до чтения с сообщением, где назван предел. Символьные ссылки внутри архива никогда не разыменовываются. Комикс распознаётся по содержимому, поэтому RAR, сохранённый как `.cbz`, открывается через 7-Zip. Расширение для браузера применяет те же пределы для архивов и документов.
