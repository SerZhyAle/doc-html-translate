...
- Коды выхода: `0` - готово, `1` - неверные аргументы, `2` - ошибка ввода-вывода, `3` - документ не удалось разобрать, `4` - перевод не удался или остановился на полпути (книга всё равно собрана и открыта, остальное на исходном языке; сообщение говорит `переведено частично, N из M страниц`), `130` - прервано по Ctrl+C (следующий запуск соберёт результат заново).
