...
- Вартість платного перекладу оцінюється як `символи / 1e6 * $20`, у символах (не байтах), за всім, що надсилається: сторінками, назвою книги й пунктами змісту. `-max-cost N` - жорстка перевірка до надсилання за будь-якого розміру документа: якщо оцінка більша за `N`, переклад пропускається, а книга все одно збирається без перекладу. Заданий ліміт - це і є згода: оцінка в межах `N` перекладається без вікна підтвердження, тож запуск без нагляду або зі скрипта (`-google -max-cost 2 -noopen book.epub`) не зупиняється на питанні. Без `-max-cost` вікно, як і раніше, питає про все, що довше за 1000 символів. Від'ємний, `NaN` або нескінченний `-max-cost` відхиляється під час запуску (код виходу `1`).
