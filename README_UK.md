...
- Коди виходу: `0` - готово, `1` - хибні аргументи, `2` - помилка введення-виведення, `3` - документ не вдалося розібрати, `4` - переклад не вдався або зупинився на півдорозі (книгу все одно зібрано й відкрито, решта мовою оригіналу; повідомлення каже `перекладено частково, N з M сторінок`), `130` - перервано через Ctrl+C (наступний запуск збере результат заново).
