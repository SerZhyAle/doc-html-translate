...
- **The word-gap split** is the only rule that runs *before* the clustering, because it repairs the
  ...
  it is 4.80x, and 3.5 is the geometric middle. The band under it **overlaps** - comic balloons drawn
  side by side stitch at 1.00-3.46x while real lines reach 3.07x - so no ratio separates it, and it is
  cut on evidence from the pixels between the two words instead: **the stroke test** (lab ticket 16,
  Phase 07 Step 07.3, 2026-09-25). `OCR_BOUNDARY_REACH = 0.14` on both sides (`tesseract.go`
  `ocrBoundaryReach` == `ocr-cluster.js`), and `TestParityOCRClustering` pins what the number is
  applied to: a gap is cut when it is too wide **or** a stroke crosses it; the reach is the words'
  median height times the constant, in the recognizer's own pixels; the path must run past the band on
  **both** sides (one side is a descender) and reach the far row; ink is the plate colours'
  `PLATE_MIN_CONTRAST` from a paper read **outside** the word boxes (bold lettering fills half of its
  own box); every pass reads the plane of the picture itself, not the grey or low-passed copy it hands
  the recognizer; and an untranslatable run a stroke cut off is an orphan that `orderColumns` parks.
  The reach is bracketed on all 1043 word gaps of the corpus with the desktop engine
  ([`DEV/research/ocr_balloon_boundary_2026-09-25.md`](../DEV/research/ocr_balloon_boundary_2026-09-25.md)):
  at 0.07 and below a real line is cut (a masthead's J, left out of its box), at 0.30 the first stitch is
  lost, and 0.14 is the geometric middle. The luminance differs by construction and not by a rule: the
  desktop reads `frame.grey()` (Go's JFIF weights), the extension `luma()` of the prepared canvas (the
  same 299/587/114 weights, truncated) - within one level of each other, and each edition's decoder
  already differs by more than that.
  **Not covered, measured:** when the recognizer reads both outlines of a balloon pair as one token of
  their own (`ff`, `fj`, `})j`) no gap holds a stroke and the line stays stitched - the lab's
  `synth-side-by-side-balloons` is built with 16 px of padding so that it does not do this, and its
  comment says why.
...
- **The confidence floor keeps a record of what it rejected** - `tesseract.go` `keepLine` +
  ...
  - **it travels with the rung that won**, not merged across the ladder's rungs, because a merged
    set describes no single decision. Across *passes* it is merged, in the same order on both
    editions (2026-09-25, before that the extension replaced the ordinary pass's drops with the
    ladder's): the ordinary pass's drops, then the ladder's (its winning rung, or the rung that read
    the most when none placed anything) or the screen sweep's. The ordinary pass ran whatever the
    ladder did afterwards, so its rejections are the reader's loss either way.
  ...
  - **display space** (OCR-OVERLAY rule 2). The desktop builds the record in the upscaled image's
    space and `scaleDown` brings it back with the plates; the extension's `collectLines` has
    already divided by the scale.

  Both editions write it as one JSON line per recognized image into an `ocr-diag.jsonl` in the same
  shape - `file`, `width`, `height`, `blocks`, `dropped` (each: `text`, `conf`, `floor`, `gate`,
  the box) - and **also for an image that produced no
...
- **Single-page merge styling and scoping.** In Go, single-page output is created by merging spine pages
  ...
  preserve each source page's head and body `<style>` blocks and `<body>` attributes without cross-chapter
  style collisions, Go wraps each chapter in `<div class="dht-chapter dht-ch-N ...">` and scopes all CSS
  rules to `.dht-ch-N` (with ID renames and relative CSS URL rebasing). The extension has no such file
  merge: it parses fragments into its shared viewer DOM, where FB2 and PDF reflow styles are provided by
  [`viewer.css`](../extension/src/viewer.css) (`#content p.stanza`, `#content p.subtitle`, etc.) and PDF
  flipped images are physically normalized on canvas extraction ([`pdf-images.js`](../extension/src/pdf-images.js)
  `paintFlips`).
...
- **Whole-page OCR on a live web page is extension-only, and there must not be a desktop
  ...
  The layer sits over the picture **as drawn**, not over the `<img>` border box (2026-09-25,
  OCR-OVERLAY rule 4): `ocr-plates.js` `pictureBox` takes border and padding off, applies
  `object-fit` (`fill` / `contain` / `cover` / `none` / `scale-down`) with `object-position` in px or
  %, follows a scaled ancestor, and cuts the layer to what the element shows (`clip-path`). A
  picture the agent cannot place truthfully - rotated, skewed or mirrored by itself or any ancestor
  (`transformRotates`), or an `object-position` it cannot read - gets no layer until it can be
  placed again: cleared, never drawn in the wrong place. Measured in Chromium against the real agent
  ([`DEV/research/page_ocr_placement_2026-09-25`](../DEV/research/page_ocr_placement_2026-09-25/README.md)).
