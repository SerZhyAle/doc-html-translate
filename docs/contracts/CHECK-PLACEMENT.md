...
scripts on the artifact they just produced), `hand-run` (a named human or agent step), `none`.
...
- [`../../tests/placement_test.go`](../../tests/placement_test.go) fails when a `gate` record is not in
  ...
  build scripts it names, or when a script that reports a `CHECK-VERDICT` verdict has no record.
