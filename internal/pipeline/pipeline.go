...
// RunContext executes the file-to-HTML pipeline. Cancelling ctx (Ctrl+C) stops it between
// steps, pages, batches and OCR images; the output then has no completion record, so the next
// run rebuilds it instead of opening it.
// Steps: [1] Check existing / Extract -> [2] Build HTML -> [3] Translate -> [4] Open browser.
func (r Runner) RunContext(ctx context.Context) (int, error) {
