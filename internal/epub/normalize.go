...
func normalizeContent(book *Book, outputDir string) {
	...
	for i := range book.Manifest {
		...
		if err := normalizeChapter(srcPath, dstPath, book.BasePath, item.Href, xhtml, lookup); err != nil {
...
// basePath the OPF directory, from which root-relative links resolve. A
// file that needs no change is left untouched on disk.
func normalizeChapter(srcPath, dstPath, basePath, fileHref string, xhtml bool, lookup func(string) (string, bool)) error {
