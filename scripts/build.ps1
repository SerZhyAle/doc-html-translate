...
# Also rewrites internal/htmlgen/favicon.ico (written next to every generated index.html, so a
# converted book carries our icon in the browser tab) and the verb / document-type ICOs.
...
# Step 4b: the artifact must carry the stamp it was built with, never the "dev" default
...
# exe, so -ocr works offline out of the box: the pinned tessdata_fast 4.0.0 build, digest
# verified, or the build fails. Other languages are downloaded on demand by the app.
